...
# Imposta la localizzazione italiana per la formattazione dei numeri
        ...
        for pattern in patterns:
            ...
            if match and len(match.groups()) >= 1:
                ...
                try:
                    ...
                    importo_formattato = format_number(importo_float)
                    valuta = match.group(2) if len(match.groups()) >= 2 and match.group(2) else "€"
                    return importo_formattato, valuta
                except ValueError:
                    continue
    except Exception as e:
            ...
            if match:
                try:
                    ...
                    if tipo_bolletta == "acqua":
                        return f"{format_number(valore)} mc"
                    elif tipo_bolletta == "energia":
                        return f"{format_number(valore)} kWh"
                    elif tipo_bolletta == "gas":
                        return f"{format_number(valore)} Smc"
                except:
                    pass
        patterns = [
            r'consumo\s*([\d\.]+)\s*kWh',
                ...
                try:
                    ...
                    if len(match.groups()) > 1 and match.group(2):
                        ...
                        if tipo_bolletta == "acqua":
                            ...
                            unita = "mc"
                    return f"{format_number(consumo)} {unita}"
                except (ValueError, IndexError):
                    continue
        fallback = re.search(r'(\d+)\s*mc\s+Importo\s+da\s+pagare', testo)
        if fallback:
            return f"{format_number(float(fallback.group(1)))} mc"
    except Exception as e:
